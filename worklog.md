...
Agent: general-purpose (landing redesign)
Task: Rebuild Orderly marketing landing page to match exact design specification

Work Log:
- Read worklog.md for context, read existing src/components/orderly/marketing.tsx (716 lines, being REPLACED), read src/components/orderly/ui.tsx for available primitives (Button, Card, Badge, Input, Label), read src/app/globals.css to confirm CSS variables (--orderly-orange: #FF6A00, --orderly-black: #0B0B0A, --orderly-charcoal: #151515, --orderly-cream: #F7F1E7, --orderly-warm-white: #FCFAF7, etc.), and verified via grep that src/app/page.tsx imports `Marketing` and renders `<Marketing onAuth={(mode) => setAuthModal(mode)} />` — so the exported component name and onAuth prop signature must be preserved.
- Wrote a brand-new src/components/orderly/marketing.tsx that exactly follows the spec's section-by-section design (Navigation → dark Hero → Stats Bar → Features → Social Proof → How It Works → dark Bottom CTA → dark Footer). Replaced the entire 716-line file with the new design. No other files created or modified.
- Brand fidelity: every orange instance is bg-[#FF6A00] (arbitrary Tailwind value) with hover bg-[#E85F00] (matches the --orderly-orange-hover variable). Hero is bg-[#0B0B0A], bottom CTA is bg-[#151515], features/how-it-works are bg-[#F7F1E7], social proof is bg-[#FCFAF7], footer is bg-[#0B0B0A]. Text colors #171717 / #62605C / #8A8782 / #D4D4D4 / #9CA3AF used as specified. WhatsApp bubbles use #FCFAF7 (incoming) and #25D366 (outgoing).
- Layout fidelity: Navigation is absolute over hero, h-[72px], max-w-[1200px] centered, with orange rounded-xl logo tile + "Orderly" wordmark (font-semibold, 20px), desktop nav links (Product / How It Works / Pricing, 14px 500 #E5E5E5), "Log in" outline button + "Book a Demo" solid orange button. Mobile hamburger uses ChevronDown (rotates 180° when open) and a backdrop-blurred drawer.
- Hero headline uses text-[clamp(3rem,6vw,4.5rem)] with font-extrabold + tracking-[-0.045em] per spec, "Fewer Empty Chairs." in white + "More Happy Guests." in #FF6A00. Subhead 20px #D4D4D4 max-w-xl. Two CTAs (Book a Demo + See How It Works). Trust line uses the literal emojis 💳 and ⏱ per spec. Right side is a WhatsApp chat mockup: white incoming bubble ("Table for 4 this Friday at 7pm?"), green outgoing bubble with the exact "Absolutely! 🎉 You're booked for Fri, 16 May at 7:00 PM. See you soon!" text + a typing indicator + a floating "Bookings today +24" accent card.
- Stats Bar overlaps hero/content via -mt-20 z-10; dark #151515 card rounded-2xl with subtle border; 4-col grid on desktop, 2-col on mobile. Stats: 25-40% More Bookings / 98% Questions Answered / 2s Average Response / 24/7 Always On — each with orange icon (TrendingUp / MessageCircle / Clock / Bell) on bg-[#FF6A00]/10, white 3xl number, gray label.
- Features section: orange uppercase tracking-wider label, H2 "Every Missed Chat Could Be a Missed Sale" on the left + body paragraph on the right (2-col grid on desktop). 5 feature cards in a row on lg (grid-cols-5), each with orange 48x48 icon circle (bg-[#FFF7ED], icon #FF6A00), 18px font-semibold title, 14px #6B7280 body. Icons: MessageCircle, Calendar, Bell, Users, BarChart3.
- Social Proof: centered label + H2 "From Sandton to Satisfying Guests" + body. 3 testimonial cards (bg-[#FAFAFA], rounded-xl, p-7) with 5 filled Star icons, the exact quote text, name + role for Marco P. / Lauren B. / Thabo M.
- How It Works: 3 steps in a row with a horizontal dashed line connecting the icon centers on desktop (absolute top-8 left-[18%] right-[18%], border-t-2 border-dashed border-[#FF6A00]/30). Each Step has a dashed-ring white-circle icon with a small orange number badge in the top-right corner. Icons: Phone (Connect) → Sliders (Customize) → TrendingUp (Convert).
- Bottom CTA: H2 "Ready to Fill More Chairs?" in #FF6A00, body, 3-item checklist with green Check icons, "Book a Demo" (orange, calls onAuth('signup')) + "Talk to Sales" (outline border-[#444]). Right side has a white dashboard preview card with the four fake metrics (Total Bookings 127, New Guests 86, Response Rate 98%, No-Show Rate 4%) and a 7-bar gradient chart.
- Footer: "TRUSTED BY LEADING RESTAURANTS" label + 5 partner names separated by bullet dots (GEMELLI SANDTON | THE CODFATHER | MARBLE RESTAURANT | HUBBLY BUBBLY | PAPPAS). Bottom bar with logo + "© 2025 Orderly. All rights reserved." + Privacy Policy / Terms of Service links + 3 social icon circles (X / in / IG). Footer has mt-auto and the root div has min-h-screen flex flex-col so the footer sticks to the bottom on short viewports.
- Prop wiring: every "Book a Demo" button (nav, hero, bottom CTA) calls onAuth('signup'). The "Log in" button (nav desktop + mobile drawer) calls onAuth('login'). The "Talk to Sales" and "See How It Works" are anchor links (# / #how-it-works) — they do not trigger auth.
- Responsive: every grid is grid-cols-1 on mobile and expands to multi-column at sm/md/lg breakpoints. Hero columns stack, hero buttons go full-width on mobile. Step connectors are desktop-only. Footer bottom bar stacks on mobile.
- No external images used — hero background is pure CSS (two orange radial blur blobs + a radial-gradient overlay). All icons are from lucide-react: MessageCircle, Calendar, Bell, Users, BarChart3, Phone, Sliders, TrendingUp, ArrowRight, Check, Star, Clock, ChevronDown. Unused primitives (Button/Card/Badge/Input/Label from ui.tsx) intentionally NOT used — raw <button>/<a>/<div> with arbitrary Tailwind values gives exact pixel control over the design spec; the shared Button primitive uses bg-primary and rounded-md which does not match the spec's rounded-lg + #FF6A00.
- Verified: `npx tsc --noEmit` reports zero errors mentioning marketing.tsx (project-wide check). `npx eslint src/components/orderly/marketing.tsx` reports zero warnings/errors. No tests written per task constraints. No other files modified.

Stage Summary:
- Orderly marketing landing page rebuilt from scratch to match the design spec exactly — 716-line legacy component replaced with a focused ~580-line design-faithful implementation.
- All 8 sections present and spec-compliant: Navigation (transparent over dark hero), dark Hero with WhatsApp chat mockup + floating accent, Stats Bar (overlapping dark card), Features (cream, 5-card row), Social Proof (warm white, 3 testimonials), How It Works (cream, 3 steps with dashed connector), Bottom CTA (dark, with dashboard preview card + fake metrics + 7-bar chart), Footer (dark, 5 partner names + bottom bar with social icons).
- Brand colors applied via Tailwind arbitrary values (bg-[#FF6A00], bg-[#0B0B0A], bg-[#F7F1E7], bg-[#FCFAF7], bg-[#151515], etc.) — exactly matching the CSS variables in globals.css.
- Auth wiring verified: all "Book a Demo" buttons → onAuth('signup'), "Log in" buttons → onAuth('login'), preserving the existing contract with src/app/page.tsx.
- TypeScript-clean, ESLint-clean. No new files. No tests. Build-safe drop-in replacement.
